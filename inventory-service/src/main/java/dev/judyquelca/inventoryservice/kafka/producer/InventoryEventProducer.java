package dev.judyquelca.inventoryservice.kafka.producer;
...
public class InventoryEventProducer {
    ...
    private final KafkaTemplate<String, InventoryUpdatedEvent> inventoryUpdatedEventKafkaTemplate;
    ...
    public InventoryEventProducer(
            ...
            KafkaTemplate<String, InventoryUpdatedEvent> inventoryUpdatedEventKafkaTemplate) {
        this.confirmedKafkaTemplate = confirmedKafkaTemplate;
        this.cancelledKafkaTemplate = cancelledKafkaTemplate;
        this.inventoryUpdatedEventKafkaTemplate = inventoryUpdatedEventKafkaTemplate;
    }
    ...
    public void publishInventoryUpdated(InventoryUpdatedEvent event){
        log.info("Publishing InventoryUPdatedEvent: productId{}, available={}", event.getProductId(),event.getAvailableStock());
        inventoryUpdatedEventKafkaTemplate.send(TOPIC_INVENTORY_UPDATED,event.getProductId().toString(),event);
    }
}
